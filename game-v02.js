(async () => {
  ...
  function makeRoadSegment(index) {
    ...
    const road = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x24282d, roughness: .83, metalness: .05 })
    );
    ...
    if (index % 3 === 0) {
      for (const side of [-1, 1]) {
        const tree = new THREE.Mesh(
          new THREE.ConeGeometry(.55, 4.5, 8),
          new THREE.MeshStandardMaterial({ color: 0x203d2e, roughness: 1 })
        );
        tree.position.set(side * (10 + Math.random() * 4), 2.1, -4 + Math.random() * 8);
        group.add(tree);
      }
    }
    return group;
  ...
  function crash() {
    ...
    one('#gameOverResult').textContent =
      Math.floor(state.score).toLocaleString('ru-RU') + ' очков · ' + (state.distance / 1000).toFixed(1) + ' км';
