(async () => {
  ...
  garageRenderer.toneMappingExposure = 1.16;
  ...
  garageCamera.position.set(5.8, 3.05, 7.4);
  ...
  const floor = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x171a1f, metalness: 0.55, roughness: 0.34 })
  );
  ...
  sunGlowCanvas.width = sunGlowCanvas.height = 256;
  const sg = sunGlowCanvas.getContext('2d');
  const grd = sg.createRadialGradient(128,128,8,128,128,128);
  grd.addColorStop(0,'rgba(255,230,170,.9)');
  grd.addColorStop(.20,'rgba(255,195,110,.55)');
  ...
  function createAsphaltTexture() {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x + (Math.random() * 45 - 22), y + 25 + Math.random() * 55);
      ctx.stroke();
    }
    ctx.globalAlpha = 1;
    ...
    tex.repeat.set(1.4, 4.8);
    tex.anisotropy = Math.min(4, renderer.capabilities.getMaxAnisotropy());
    return tex;
  }

  function createBrandBillboardTexture(variant = 0, vertical = false) {
    const cv = document.createElement('canvas');
    cv.width = vertical ? 512 : 1024;
    cv.height = vertical ? 900 : 440;
    const ctx = cv.getContext('2d');
    ...
    if (dark) {
      ...
      grad.addColorStop(.58, '#111821');
      grad.addColorStop(1, '#202631');
    } else {
      grad.addColorStop(0, '#0f5c44');
      grad.addColorStop(1, '#0a3e31');
    }
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, w, h);

    // speed stripe / premium accent
    ctx.fillStyle = '#ffc400';
    ctx.fillRect(0, 0, vertical ? 18 : 24, h);
    ctx.fillRect(0, h - (vertical ? 16 : 18), w, vertical ? 16 : 18);

    ctx.globalAlpha = .11;
    ctx.fillStyle = '#ffffff';
    for (let i = -2; i < 9; i++) {
      ctx.save();
      ctx.translate(i * 170, h * .72);
      ctx.rotate(-.45);
      ctx.fillRect(0, 0, 74, h * .55);
      ctx.restore();
    }
    ctx.globalAlpha = 1;

    const pad = vertical ? 46 : 54;
    ctx.fillStyle = '#ffc400';
    ctx.font = vertical ? '900 48px -apple-system,Arial,sans-serif' : '900 60px -apple-system,Arial,sans-serif';
    ctx.fillText('ЦЕНТР', pad, vertical ? 108 : 102);
    ctx.fillStyle = '#ffffff';
    ctx.font = vertical ? '900 54px -apple-system,Arial,sans-serif' : '900 70px -apple-system,Arial,sans-serif';
    ctx.fillText('АВТОАПГРЕЙДА', pad, vertical ? 174 : 174);

    ctx.fillStyle = dark ? '#f4c329' : '#ffffff';
    ctx.font = vertical ? '800 28px -apple-system,Arial,sans-serif' : '800 32px -apple-system,Arial,sans-serif';
    ctx.fillText(
      vertical ? 'ПРОКАЧАЙ АВТОМОБИЛЬ' : 'ПРОКАЧАЙ АВТО В РЕАЛЬНОСТИ',
      pad,
      vertical ? 254 : 236
    );

    ctx.fillStyle = '#e9edf2';
    ctx.font = vertical ? '700 28px -apple-system,Arial,sans-serif' : '700 30px -apple-system,Arial,sans-serif';
    if (vertical) {
      ['Android • Автозвук', 'Камеры • Доводчики', 'Симферополь', 'ул. Воровского, 19'].forEach((line, i) => {
        ctx.fillText(line, pad, 338 + i * 54);
      });
      ctx.fillStyle = '#ffc400';
      ctx.font = '900 34px -apple-system,Arial,sans-serif';
      ctx.fillText('+7 (978) 046-58-81', pad, 590);
    } else {
      ctx.fillText('Симферополь • ул. Воровского, 19', pad, 302);
      ctx.fillStyle = '#ffc400';
      ctx.font = '900 42px -apple-system,Arial,sans-serif';
      ctx.fillText('+7 (978) 046-58-81', pad, 362);
      ctx.fillStyle = '#cbd2da';
      ctx.font = '700 22px -apple-system,Arial,sans-serif';
      ctx.fillText('ANDROID • АВТОЗВУК • КАМЕРЫ • ДОВОДЧИКИ', pad, 407);
    }
  ...
  function makeBrandBillboard(variant = 0, side = 1, vertical = false) {
    const group = new THREE.Group();
    const tex = createBrandBillboardTexture(variant, vertical);
    const board = new THREE.Mesh(
      new THREE.PlaneGeometry(vertical ? 3.1 : 7.5, vertical ? 5.3 : 3.2),
    ...
    board.position.y = vertical ? 4.1 : 4.0;
    group.add(board);

    const metal = new THREE.MeshStandardMaterial({ color: 0x8f969d, metalness: .72, roughness: .3 });
    const xs = vertical ? [0] : [-2.7, 2.7];
    xs.forEach((x) => {
      const post = new THREE.Mesh(new THREE.BoxGeometry(.14, vertical ? 4.2 : 4.0, .14), metal);
      post.position.set(x, vertical ? 2.05 : 1.95, .05);
      group.add(post);
    });

    group.position.x = side * (vertical ? 10.5 : 10.0);
    group.rotation.y = side < 0 ? .03 : -.03;
    return group;
  }

  function makeCypress(height = 5) {
    const group = new THREE.Group();
    const trunk = new THREE.Mesh(
      new THREE.CylinderGeometry(.12, .16, height * .42, 7),
      new THREE.MeshStandardMaterial({ color: 0x493528, roughness: 1 })
    );
    trunk.position.y = height * .21;
    group.add(trunk);
    const crown = new THREE.Mesh(
      new THREE.ConeGeometry(.58, height, 10),
      new THREE.MeshStandardMaterial({ color: 0x173b2d, roughness: 1 })
    );
    crown.position.y = height * .58;
    crown.scale.x = .72;
    group.add(crown);
    return group;
  }
  ...
  function makeBridge() {
    const group = new THREE.Group();
    const concrete = new THREE.MeshStandardMaterial({ color: 0x8f9294, roughness: .86 });
    const deck = new THREE.Mesh(new THREE.BoxGeometry(18, .65, 3.3), concrete);
    deck.position.y = 5.0;
    group.add(deck);
    [-7.5, 7.5].forEach((x) => {
      const p = new THREE.Mesh(new THREE.BoxGeometry(.75, 5.0, .75), concrete);
  ...
  function makeRoadSegment(index) {
    ...
    const edgeLineMat = new THREE.MeshBasicMaterial({ color: 0xf6f2df });
    [-6.0, 6.0].forEach((x) => {
      const edge = new THREE.Mesh(new THREE.PlaneGeometry(.12, 28), edgeLineMat);
      edge.rotation.x = -Math.PI / 2;
      edge.position.set(x, .03, 0);
      group.add(edge);
    });

    // road reflectors for speed perception
    ...
    [-6.55, 6.55].forEach((x) => {
      for (let z = -11; z <= 11; z += 7) {
        const r = new THREE.Mesh(new THREE.BoxGeometry(.08, .06, .22), reflectorMat);
        r.position.set(x, .10, z);
        group.add(r);
      }
    });
    ...
    // Center Auto Upgrade branded track placements
    if (index === 1) group.add(makeBrandBillboard(0, -1, false));
    if (index === 4) group.add(makeBrandBillboard(1, 1, false));
    if (index === 7) group.add(makeBrandBillboard(0, 1, true));
    if (index === 11) group.add(makeBrandBillboard(1, -1, false));
    if (index === 13) group.add(makeBrandBillboard(0, -1, true));

    if (index % 2 === 0) {
      for (const side of [-1, 1]) {
        const tree = makeCypress(3.8 + Math.random() * 2.6);
        tree.position.set(side * (9.5 + Math.random() * 5.5), 0, -7 + Math.random() * 14);
        group.add(tree);
      }
    }
    return group;
  ...
  function raceLoop(now) {
    ...
    if (!state.paused) {
      ...
      camera.position.y = 4.0 + speedRatio * .25;
