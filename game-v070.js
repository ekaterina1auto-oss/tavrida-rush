(async () => {
  ...
  function makeIdentityCar(type, color, scale = 1) {
    ...
    if(type==='bmw'){
      // BMW-like double kidney grille.
      [-.23,.23].forEach(x=>{
        const kidney=addBox(.34,.42,.07,dark,x,cfg.ride+.70,frontZPlane);
        const outline=addBox(.40,.48,.035,trim,x,cfg.ride+.70,frontZPlane-.03);
        const inner=addBox(.31,.38,.04,dark,x,cfg.ride+.70,frontZPlane-.05);
      });
      // slim angular headlights with inner light signatures
      [-.66,.66].forEach(side=>{
        addBox(.56,.12,.055,headMat,side*.62,cfg.ride+.91,frontZPlane);
        addBox(.24,.035,.058,dark,side*.62,cfg.ride+.91,frontZPlane-.01,0,0,side*.28);
      });
      addBox(1.55,.18,.06,dark,0,cfg.ride+.49,frontZPlane);
      // rear L-style light clusters
      [-.62,.62].forEach(side=>{
        addBox(.64,.11,.05,tailMat,side*.59,cfg.ride+.90,rearZPlane);
        addBox(.12,.23,.05,tailMat,side*.84,cfg.ride+.83,rearZPlane);
      });
      addBox(1.58,.06,.17,dark,0,cfg.ride+.48,rearZPlane-.08);
      [-.55,.55].forEach(x=>{
        const tip=new THREE.Mesh(new THREE.CylinderGeometry(.09,.09,.20,12),trim);
        tip.rotation.x=Math.PI/2; tip.position.set(x,cfg.ride+.39,rearZPlane+.07); group.add(tip);
      });
    }

    if(type==='vesta'){
      // X-face: crossing satin bars and central grille.
      addBox(1.02,.32,.075,dark,0,cfg.ride+.64,frontZPlane);
      [-1,1].forEach(side=>{
        const a=addBox(.10,.70,.06,trim,side*.48,cfg.ride+.78,frontZPlane-.01,0,0,side*.58);
        const b=addBox(.10,.70,.06,trim,side*.48,cfg.ride+.78,frontZPlane-.01,0,0,-side*.58);
        addBox(.50,.15,.055,headMat,side*.62,cfg.ride+.98,frontZPlane,0,0,-side*.10);
      });
      // Widebody arches / side skirts.
      [-1,1].forEach(side=>{
        addBox(.18,.35,1.05,paintDark,side*(cfg.w/2+.11),cfg.ride+.47,frontZ);
        addBox(.18,.35,1.05,paintDark,side*(cfg.w/2+.11),cfg.ride+.47,rearZ);
        addBox(.16,.17,2.25,dark,side*(cfg.w/2+.08),cfg.ride+.29,0);
      });
      // Big wing, diffuser and twin exhausts.
      addBox(2.24,.09,.32,dark,0,cfg.ride+1.44,rearZPlane-.25);
      [-.68,.68].forEach(x=>addBox(.08,.55,.08,dark,x,cfg.ride+1.20,rearZPlane-.43));
      addBox(1.78,.18,.24,dark,0,cfg.ride+.34,rearZPlane-.08);
      [-.56,.56].forEach(x=>{
        const tip=new THREE.Mesh(new THREE.CylinderGeometry(.105,.105,.25,12),trim);
        tip.rotation.x=Math.PI/2; tip.position.set(x,cfg.ride+.37,rearZPlane+.08); group.add(tip);
      });
      // angular taillights
      [-.62,.62].forEach(side=>{
        addBox(.62,.10,.05,tailMat,side*.60,cfg.ride+.94,rearZPlane,0,0,side*.13);
      });
      const underGlow=new THREE.PointLight(0xff2518,11,7,2); underGlow.position.set(0,.16,0); underGlow.name='underGlow'; group.add(underGlow);
      const glow=new THREE.Mesh(new THREE.CircleGeometry(1.75,32),new THREE.MeshBasicMaterial({color:0xff2418,transparent:true,opacity:.20,depthWrite:false}));
  ...
  function makeIdentityCarV2(type, color, scale=1) {
    ...
    const spec = {
      bmw:{
        halfW:1.00,wheelR:.40,wheelbase:2.82,ride:.40,
        body:[
          {z:-2.34,w:.82,bot:.39,top:.65},{z:-2.05,w:.94,bot:.39,top:.76},
          {z:-1.42,w:1.00,bot:.39,top:.86},{z:-.55,w:1.01,bot:.39,top:.92},
          {z:.55,w:1.01,bot:.39,top:.94},{z:1.48,w:.99,bot:.39,top:.87},
          {z:2.25,w:.88,bot:.40,top:.75}
        ],
        cabin:[
          {z:-.78,w:.72,bot:.84,top:.96},{z:-.42,w:.80,bot:.86,top:1.34},
          {z:.56,w:.79,bot:.88,top:1.38},{z:1.00,w:.68,bot:.84,top:1.05}
        ]
      },
      vesta:{
        halfW:1.03,wheelR:.40,wheelbase:2.63,ride:.42,
        body:[
          {z:-2.24,w:.82,bot:.40,top:.69},{z:-1.95,w:.98,bot:.40,top:.81},
          {z:-1.28,w:1.03,bot:.40,top:.90},{z:-.45,w:1.04,bot:.40,top:.98},
          {z:.62,w:1.04,bot:.40,top:.99},{z:1.38,w:1.02,bot:.40,top:.91},
          {z:2.20,w:.91,bot:.41,top:.79}
        ],
        cabin:[
          {z:-.80,w:.73,bot:.90,top:1.02},{z:-.36,w:.83,bot:.91,top:1.39},
          {z:.66,w:.82,bot:.92,top:1.43},{z:1.08,w:.69,bot:.88,top:1.10}
        ]
      },
      amg:{
    ...
    const body=loftShell(spec.body,paint);
    group.add(body);
    const cabin=loftShell(spec.cabin,glass);
    group.add(cabin);

    // roof cap and side sills
    const roofZ=type==='porsche'?.45:.30;
    const roofW=type==='porsche' ? .63 : .69;
    const roofY=type==='porsche' ? 1.28 : 1.40;
    const roof=new THREE.Mesh(new THREE.BoxGeometry(roofW*2,.055,type==='porsche'?.82:1.04),paint);
  ...
  let garageCar = makeGameCar('bmw', carSpecs['BMW M340i'].color, 1.13);
  ...
  function rebuildGarageCar() {
    ...
    garageCar = makeGameCar(spec.type, spec.color, 1.13);
    garageCar.rotation.y = -0.6;
    garageScene.add(garageCar);
  }
  ...
  function rebuildPlayer() {
    ...
    const spec = carSpecs[state.car];
    player = makeGameCar(spec.type, spec.color, .84);
  ...
  function applyRaceTheme() {
    if (state.track === 'Ночной Симферополь') {
      ...
      scene.fog = new THREE.Fog(0xd39168, 42, 205);
      ...
      scene.background = heroSkyTexture;
      scene.fog = new THREE.Fog(0xb88768, 42, 198);
  ...
  function raceLoop(now) {
    ...
    if (!state.paused) {
      ...
      camera.fov = 54 + speedRatio * 10;
      camera.position.x += ((playerX * .16) - camera.position.x) * Math.min(1, dt * 3.8);
      camera.position.y = 3.12 + speedRatio * .18 + Math.sin(now * .012) * .018 * speedRatio;
      camera.position.z = 8.70 - speedRatio * .34;
      camera.rotation.z += (((playerTargetX - playerX) * -.006) - camera.rotation.z) * Math.min(1, dt * 4);
      camera.lookAt(playerX * .12, .72, -10.5);
      camera.updateProjectionMatrix();
