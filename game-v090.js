  ...
  function raceLoop(now) {
    ...
    if (!state.paused) {
      ...
      traffic.forEach((car) => {
        ...
        const targetX = laneX[car.userData.targetLane];
      ...
      camera.position.x += ((playerX * .48) - camera.position.x) * Math.min(1, dt * 5.0);
      ...
      camera.lookAt(playerX * .34, .72, -10.5);
      camera.updateProjectionMatrix();
